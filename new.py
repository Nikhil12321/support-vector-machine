...
number_of_documents = 200.0  # number of training sets
...
newfeatureList = get_feature_list()
inp = csv.reader(open('debate.csv', 'rb'), delimiter=',')
stopwords = getStopWords()
tweets = []
featureVector = []
documents_feature_list = []
...
for row in inp:
    ...
    for w in featureVector:
        documents_feature_list.append(w)


global_vector = get_global_vector(documents_feature_list)
...
if __name__ == "__main__":
    ...
    def gen_lin_separable_data():
        ...
        X1 = np.array(weight_feature_vector[:200])
        y1 = np.ones(len(X1))
        X2 = np.array(weight_feature_vector[1304:1504])
